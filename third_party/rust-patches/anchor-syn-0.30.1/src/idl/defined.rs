...
fn call_site_source_path() -> Option<PathBuf> {
    env::var("ANCHOR_IDL_BUILD_PROGRAM_PATH")
        .ok()
        .map(PathBuf::from)
        .map(|root| root.join("src/lib.rs"))
}
...
pub fn gen_idl_type(
    ...
    match ty {
        ...
        syn::Type::Path(path) => {
            ...
            let source_path_opt = call_site_source_path();

            // Handle type aliases and external types
            if let Some(source_path) = source_path_opt.clone() {
